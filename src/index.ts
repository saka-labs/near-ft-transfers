...
const validator = new AccountValidator(env.nearRpcUrl, env.nearContractId, {
...
const getTransfersRoute = createRoute({
  ...
  description:
    "Get all transfers with optional filtering by receiver_account_id or is_stalled status",
  request: {
...
app.openapi(unstallTransferRoute, async (c) => {
  ...
  return c.json(
    { success: true, message: "Transfer unstalled successfully" },
    200,
  );
});
...
const unstallTransfersRoute = createRoute({
  ...
  description:
    "Release multiple stalled transfers or all stalled transfers back to pending status",
  request: {
    body: {
      content: {
        "application/json": {
...
app.doc("/doc", {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "API for queuing and managing NEAR fungible token (FT) transfers with batch processing support",
  },
  tags: [
    {
      name: "Transfers",
